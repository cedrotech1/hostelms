...
// --- 1. Fetch hostels assigned to this wadden ---
$hostels = [];
$hostel_query = "SELECT h.id, h.name, h.building_code FROM hostels h JOIN wadden_hostels wh ON h.id = wh.hostel_id WHERE wh.wadden_id = ? ORDER BY h.name";
...
$count_query = "SELECT COUNT(*) as total
                ...
                JOIN wadden_hostels wh ON wh.hostel_id = h.id
                $where";
...
$status_stats_query = "SELECT c.status, COUNT(*) as count FROM claiming c
    ...
    JOIN hostels h ON r.hostel_id = h.id
    JOIN wadden_hostels wh ON wh.hostel_id = h.id
    WHERE wh.wadden_id = ? GROUP BY c.status";
...
$category_stats_query = "SELECT category, COUNT(*) as count FROM claiming c
    JOIN rooms r ON c.room_id = r.id
    JOIN hostels h ON r.hostel_id = h.id
    JOIN wadden_hostels wh ON wh.hostel_id = h.id
    WHERE wh.wadden_id = ? GROUP BY category";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['archive_claim'])) {
    ...
    $stmt->execute();
    echo "<script>window.location.href='manage_claims.php" . (isset($_GET['archived']) ? "?archived=1" : "") . "';</script>";
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['reply_claim'])) {
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Claim updated successfully!'); window.location.href='manage_claims.php';</script>";
        exit();
    } else {
        ...
        <section class="section">
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">All Room Claims</h5>
                            ...
                            <div class="mb-3">
                                <?php if ($show_archived): ?>
                                    <a href="manage_claims.php" class="btn btn-outline-primary"><i class="bi bi-arrow-left"></i> View Active Claims</a>
                                <?php else: ?>
                                    <a href="manage_claims.php?archived=1" class="btn btn-outline-secondary"><i class="bi bi-archive"></i> View Archived Claims</a>
                                <?php endif; ?>
                            </div>
                                                            ...
                                                            <form method="POST">
                                                                <div class="modal-body">
                                                                    ...
                                                                    <div class="mb-3">
                                                                        ...
                                                                        <input type="text" class="form-control" value="<?php echo $claim['student_name'] . ' (' . $claim['regnumber'] . ')'; ?>" readonly>
                                                                    </div>
                                                                    
                                                                    <div class="mb-3">
                                                                        <label class="form-label">Room</label>
                                                                        <input type="text" class="form-control" value="<?php echo $claim['hostel_name'] . ' - ' . $claim['room_code']; ?>" readonly>
                                                                    </div>
